...
describe('Duration', () => {
  ...
  describe('Number of notes to fill measure in 4/4', () => {
    ...
    test('One Whole note', () => {
      expect(Duration.Whole.toFill(timeSignature)).toBe(1);
    });

    test('Two Half notes', () => {
      expect(Duration.Half.toFill(timeSignature)).toBe(2);
    });

    test('Four Quarter notes', () => {
      expect(Duration.Quarter.toFill(timeSignature)).toBe(4);
    });

    test('Eighth Eighth notes', () => {
      expect(Duration.Eighth.toFill(timeSignature)).toBe(8);
    });
  });
});
